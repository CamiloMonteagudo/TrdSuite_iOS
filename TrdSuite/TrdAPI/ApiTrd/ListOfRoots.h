...
class CListOfRoots : public CTranslationDataObject, public CPtrArray
  ...
  public:
    CListOfRoots(CTranslationUser *ptrTranslationUser,CPtrArray *ptrArrayItem, LPCWORD ptrWord):CTranslationDataObject(ptrTranslationUser)
      {
  ...
  protected:
    bool GetTranslation();                     // 
    int CompareItem();
    CPtrArray *GetPtrArray()  // arreglo de reducciones para la palabra de trabajo
      {return m_ptrArray;}
    ...
    STranslationString *GetArrayItem(int iPos)  // arreglo de reducciones para la palabra de trabajo
      {return (STranslationString *)m_ptrArray->GetAt(iPos);}
    ...
    CDict *GetDictionary()                   // obtine el diccionario general de palabras
      {
