...
void CReflexiveVerbInSentence::PutReflexivePronoun(LPCWORD ptrWord)
  ...
  int iTargetLanguage = CLangIso::Id(GetPtrTargetLanguageData()->GetIsoName());
	...
	if(ptrWord->m_Persona == SEGUNDAP && iTargetLanguage == 4)  // para el francés el reflexivo es diferente para cada persona no hay segunda de respeto
    ...
    if (ptrWord->m_Reflexivo == TRUE && GetPtrTargetLanguageData()->GetGerundType().Find(ptrWord->m_sTipo) != -1 && ptrWord->m_Modo == mGERUNDIO && (iTargetLanguage == 0))
        ...
        CStringA tmp;
        tmp += ptrWord->m_Trad;
        CSGrepArray GrepRef((iTargetLanguage == 2)?RefInfGrepArrayIt:(iTargetLanguage == 4)?RefInfGrepArrayFr:RefInfGrepArrayEs,TRUE,FALSE);
        GrepRef.Replace(tmp,FALSE);
		tmp.Replace("#",(iTargetLanguage == 2)?RefArrayIt[i]:(iTargetLanguage == 4)?RefArrayFr[i]:RefArrayEs[i]);
        ptrWord->m_Trad = tmp;
        }    
    else if(   ptrWord->m_Reflexivo == TRUE  
		...
		if(ptrWord->m_Trad.Find(' ') == -1)   // solo lo puedo hacer si es palabra ya que hay que buscar el verbo en la frase y no lo hago todavía
			...
			tmp += " ";
			tmp += ((iTargetLanguage == 2)?RefArrayIt[i]:(iTargetLanguage == 4)?RefArrayFr[i]:RefArrayEs[i]);
	        ptrWord->m_Trad = tmp;
			}
        }    
    else if(   ptrWord->m_Reflexivo == TRUE  
        ...
        ptrWord->m_Trad = tmp;
		****/
        CStringA tmp((iTargetLanguage == 2)?RefArrayIt[i]:(iTargetLanguage == 3)?RefArrayDe[i]:(iTargetLanguage == 4)?RefArrayFr[i]:RefArrayEs[i]);

        if(iTargetLanguage != 3)
            ptrWord->m_Trad = tmp + " " + ptrWord->m_Trad;
        else
            ptrWord->m_Trad = ptrWord->m_Trad + " " + tmp;
        }    
