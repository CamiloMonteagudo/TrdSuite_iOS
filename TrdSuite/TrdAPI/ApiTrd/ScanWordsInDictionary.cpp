    ...
    if ( bFind == FALSE || GetInsertAllWord() == true)
      ...
      if(GetDictionary()->IsKey(ptrWord->m_Orig) == TRUE)
				...
				if(ptrWord->m_Orig.IsEmpty() == FALSE && ptrWord->m_Orig.Find(' ') == -1)
					...
					sKey = ptrWord->m_Origlwr;
				else /*if(ptrWord->m_Key.IsEmpty() == FALSE && ptrWord->m_Key.Find(' ') == -1)*/
					sKey = ptrWord->m_Key;
				}

			UpdateFrecuency(sKey,false,true,false/*GetUpdateDic()*/);
      }
    }
...
bool CScanWordsInDictionary::FindKeySufijo(LPCWORD ptrWord,POSITION nodo)
    ...
    for(;;)    
        {
        ret = (GetPtrSourceLanguageData()->GetPtrSuffixReductionDic()->FindSuffix(findkey,ptrWord->m_Data))?true:false;
