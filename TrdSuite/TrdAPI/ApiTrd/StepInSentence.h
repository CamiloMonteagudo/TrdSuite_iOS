...
protected:
	...
	CDict   	  *m_ptrDic;				  // diccionario de trabajo
...
protected:
	...
	virtual CDict *GetDictionary()
		{return m_ptrDic;}
	inline void SetDictionary(CDict *ptrDic = NULL)
		{m_ptrDic = (ptrDic != NULL)?ptrDic:GetTranslationUserPtr()->GetPtrTranslationDic();}

	virtual void UpdateFrecuency(const CStringA& ptrKey, bool bToLower = false, bool bUpdateDest = true, bool bUpdateDic = false)
		{
		CStringA sKey(ptrKey);
		CStringA sData,sField;
		DWORD iNumOfWords = 0;

		if(bToLower == true)
		{
			sKey.MakeLower();
			//_strlwr( (LPSTR)(LPCSTR)sKey );
		}

		if(sKey.IsEmpty() == FALSE)
			{
			if(GetDictionary() != NULL && bUpdateDic == true)
				{
				ASSERT(FALSE); // aqui nunca entra
				if(GetDictionary()->GetKey(sKey,sData) == TRUE)
					{             
					GetSubCampo(sField,sData,'F','#');
					iNumOfWords = atoics(sField);
					}
				iNumOfWords++;
				sField.FromInt((int)iNumOfWords);
				//SetSubCampo(sData,sField,'F','#');
				ASSERT(FALSE); // --> Insert(sKey,sData); no esta implementada
				//GetDictionary()->Insert(sKey,sData);
				}
			if(bUpdateDest)
				{
