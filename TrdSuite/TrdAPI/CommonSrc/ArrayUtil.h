...
public:
	...
	inline const TYPE& GetAt(TInt aIndex) const { return pData[aIndex]; }
	...
	void InsertAt(TInt aIdx, const TYPE& aValue) 
		...
		// inicializar en cero
		char* pBuf = (char*)(pData + aIdx);
		for(TUint i=0; i<sizeof(TYPE); i++, pBuf++)
			*pBuf = 0;
		//memclr()
		// llamar al copy constructor
...
public:
	...
	void Preallocate(TInt aNumItems, TInt sItemLen)
		{
		DWORD aStrTotalLen = aNumItems * (sItemLen + 1);
	...
	void Add(const std::string& aDesc)
		{
		DWORD newLen = iBuffer.length() + 1 + aDesc.length();
		if (newLen > iBuffer.capacity())
			{
			TInt expandSize = (int)Max(1000, newLen/2);
		...
		iBuffer.append(aDesc);
		}
	
	void Add(LPCSTR aStr, int aLen)
		{
		DWORD newLen = iBuffer.length() + 1 + aLen;
		if (newLen > iBuffer.capacity())
			{
			TInt expandSize = (int)Max(1000, newLen/2);
			iBuffer.reserve(newLen + expandSize);
