...
#if defined(_WINDOWS_) || defined(_IPHONE_) || defined(_BLACKBERRY_)
	typedef typename _ST::_string _string;
#else	
	typedef class _ST::_string _string;
#endif	
...
#define TRACE_DEST
	...
	TStringDataT(TNullStrData):iRefCount(1000000000),iOrgBuffer()
		{TRACE_CONST; 
		}
	...
	inline TInt GetRefCount()   { return iRefCount; }

	inline bool IsExclusive() 	{ return (iRefCount == 1); }
	
	inline _string& GetDes()		{ return iOrgBuffer; }
	inline _string& GetString()	{ return iOrgBuffer; }
	
	inline const _string* GetStringC() 	{ return &iOrgBuffer; }
	
	inline const _char* c_str()
		{
		return iOrgBuffer.c_str();
		}

	inline const _char* Data()
		{
...
#define RELEASE_DATA if (iData)	iData->Release()
...
public:
	...
	explicit CMFCString(const _char* aStr1, const _char* aStr2):iData(NULL)
		...
		iData->GetDes().assign(aStr1, len1);
		iData->GetDes().append(aStr2, len2);
		}

	explicit CMFCString(_char aCh, const _char* aStr2):iData(NULL)
		{
		iData = new _TStringData(1 + strlen(aStr2));
		iData->GetDes().assign(1, aCh);
		iData->GetDes().append(aStr2);
		}
...
public:
	...
	friend inline CMFCString operator+(const CMFCString& aStr1, const CMFCString& aStr2)
		...
		CMFCString res(len);
		res.GetDesEx().assign(aStr1);
		res.GetDesEx().append(aStr2);
		return res;
		}
	...
	friend inline CMFCString operator+(const _char* aStr1, const CMFCString& aStr2)
		...
		res.GetDesEx().assign(aStr1, len1);
		res.GetDesEx().append(aStr2);
		return res;
		}
	friend inline CMFCString operator+(const CMFCString& aStr1, const _char* aStr2)
		...
		res.GetDesEx().assign(aStr1);
		res.GetDesEx().append(aStr2, len2);
		return res;
		}
	...
	//inline operator _TPtrC() const { return _TPtrC(*GetDesC()); };
	...
	void Copy(_char aChar)
		...
		if (!IsExclusive())
			{
			RELEASE_DATA;
			iData = new _TStringData(10);
			}
		
		GetDesEx().assign(1, aChar);
		AFTER_MODIFY;
		}

	// Los CopyC son para ser usados desde el constructor
...
// ************************************************
	...
	inline TInt GetRefCount()
		{
		return iData->GetRefCount();
		}
	
	// Devuelve un TDesC* que puede usarse para acceder a la cadena directamente 
	// para solo lectura
	...
	CMFCString Mid(TInt aPos, TInt aLen) const
		...
		CMFCString res(GetDesC()->substr(aPos, aLen));
	...
	CMFCString Mid(TInt aPos) const
		...
		CMFCString res(GetDesC()->substr(aPos));
	...
	CMFCString Left(TInt aLen) const
		...
		if (aLen >= thisLen)
			...
			{
			CMFCString res(str->substr(0, aLen));
			return res;
			}
		}
	...
	CMFCString Right(TInt aLen) const
		...
		if (aLen >= thisLen)
			...
			{
			CMFCString res(str->substr(thisLen - aLen, aLen));
			return res;
			}
		}
...
// ************************************************	
	...
	_char* GetBuffer(TInt aLen)
	  {
	  Preallocate(aLen);
		GetDesEx().resize(aLen, 0); //TODO: mejorar y averiguar porque hace falta esto, el Preallocate debia bastar
  ...
  typedef typename _string::size_type size_type;
	...
	void Enclose(const _char aChar)
		...
		GetDesEx().insert((size_type)0, (size_type)1, aChar);
		GetDesEx().append((size_type)1, aChar);
		}
