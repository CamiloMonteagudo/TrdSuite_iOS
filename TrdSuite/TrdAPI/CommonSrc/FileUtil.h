...
public:	
	...
	CFile(const CStringA& aFileName, TUint aMode)
		{
		iFile = NULL;
		iLength = 0;
	...
	enum OpenFlags { // nota: no todos estan implementados
	...
	inline bool Open(const CStringA& aFileName, TUint aMode = 0)
		{
		return Open(aFileName.c_str(), aMode);
		}
	
	bool Open(const char* aFileName, TUint aMode = 0)
		{
	...
	TInt Seek(TInt aOffset, TInt aFrom = begin)
		...
		int res = fseek(iFile, aOffset, mode);
		ASSERT(res == 0);
...
public:
	...
	inline CArchive& operator>>(TUint16& aValue) { aValue = iFile->ReadUint16L(); return *this; };
	inline CArchive& operator>>(DWORD& aValue) { aValue = iFile->ReadUint32L(); return *this; };
	inline CArchive& operator>>(TInt& aValue) { aValue = iFile->ReadInt32L(); return *this; };
	inline CArchive& operator>>(TUint8& aValue) { aValue = iFile->ReadUint8L(); return *this; };
	inline CArchive& operator>>(TInt8& aValue) { aValue = iFile->ReadInt8L(); return *this; };
