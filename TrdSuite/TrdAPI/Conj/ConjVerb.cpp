...
CString CConjVerb::ConjugaVerb( const CString& verboi, Modo modo, Tiempo tiempo, Persona persona, Numero numero, bool negado, bool compuesto)
  ...
  int col = -1;
	...
	if( (compuesto == FALSE) || (modo == mIMPERATIVO) || (modo == mINFINITIVO) || (modo == mPARTICIPIO) || (modo == mGERUNDIO))
		...
		if(!prefreflex.IsEmpty() && m_Lang != 3)
			{
			verbsimp = prefreflex;
			verbsimp += " ";
			verbsimp += verbsimp;
...
CStringA CConjVerb::DeclineNoun( const CStringA& noun, Genero /*gen*/, Numero num, Declination decl)
	...
	if( m_Lang == 3 )
		...
		if(num == PLURAL)
			...
			if(decl == DATIVE)
				{
				char last = strnoun[strnoun.GetLength()-1];
				if((last != 'n') && (last != 's'))
					declNoun += 'n';
				}
			}
			...
			CDict *DecNounIDic = m_Dicts->GetPtrIrregDecNounDict(); 

			if( DecNounIDic == NULL )
				return CStringA();

			if( DecNounIDic->GetKey(strnoun,data)==FALSE )
				{	
				//Se busca el sust. en el dicc de Sufijos de Decl. de sust.
				CDict *DecNounSufDic = m_Dicts->GetPtrSuffixDecNounDict(); 

				if( DecNounSufDic == NULL )
					return CStringA();

				if( DecNounSufDic->FindSuffix(strnoun,data)==TRUE)
					{
					int pos;

					CString sufElem;
					CString suf,declList;
					pos = data.Find(",");
					suf = data.Left(pos);
					declList = data.Mid(pos+1);

					sufElem = GetDeclElement(suf,declList,decl,0);

					data = strnoun + "#";
					pos = data.Find(suf+"#");
					declNoun = data.Left(pos) + sufElem;
					}
...
CStringA CConjVerb::DeclineAdjective( const CStringA& adj, Genero gen, Numero num, Declination decl, ArticleType artic)
	...
	if( m_Lang == 3 )
		{
		TDictData data;
		//Se busca el adj. en el dicc de Decl.Irreg de Adj.
		CDict *DecAdjIDic = m_Dicts->GetPtrIrregDecAdjDict(); 

		if( DecAdjIDic == NULL )
			return CStringA();

		if( DecAdjIDic->GetKey(adj,data)==FALSE )
			{	
			//Se busca el adj. en el dicc de Sufijos de Decl. de adj.
			CDict *DecAdjSufDic = m_Dicts->GetPtrSuffixDecAdjDict(); 

			if( DecAdjSufDic == NULL )
				return CStringA();

			if( DecAdjSufDic->FindSuffix(adj,data)==TRUE)
				{
				int pos;
				CString sufElem;
				pos = data.Find(",");
				CString suf = data.Left(pos);
				CString declList = data.Mid(pos+1);
			...
			if(data != "-")
				...
				int idx = GetDeclIndex(gen,num);
				declAdj = GetDeclElement(adj,declList,decl,idx);
				}
			}
		}
		...
		if(num == PLURAL)
			...
			if( DecNounIDic->GetKey(strnoun,data)==FALSE )
				...
				if( DecNounSufDic->FindSuffix(strnoun,data)==TRUE)
					...
					CStringA suf,declList,sufElem;
					pos = data.Find(",");
					suf = data.Left(pos);
					declList = data.Mid(pos+1);

					sufElem = GetDeclElement(suf,declList,decl,idx,decCount);
				...
				{
				declNoun = GetDeclElement(strnoun,data,decl,idx,decCount);
				}
			}
		}
