...
+(bool) LoadConjLang:(int) lng
  {
  if( _CData.CVerb != NULL )
    {
    if( _CData.cLang  == lng ) return true;
    delete _CData.CVerb;                                                    // Borra el objeto
    }
  
  CConjVerb *CVerb = new CConjVerb();                                       // Crea objeto para conjugar
  _CData.CVerb = CVerb;                                                     // Guarda objeto detro de los datos

  CVerb->SetDataPath( getDictPath() );                                      // Pone el camino a los diccionarios
  bool res = CVerb->Open( lng );                                            // Abre la conjugación
...
+(bool) ConjVerb: (NSString *)sVerb
  ...
  for( int i=0; i<_CData.Count; ++i )
    ...
    if( _CData.cLang == 1 && cnj.mode >= mMODO_FINAL )                      // Es un modo continuado del inglés
      ...
      {
      cnj.Conj = _CData.CVerb->ConjugaVerb( Verb, cnj.mode, cnj.time, cnj.Per, cnj.num, FALSE, cnj.comp );
...
bool OpenRedAPI( int lng )
  {
  if( _RedData.Red != NULL && _RedData.rLang == lng  )      // Si esa reducción ya estaba abierta
    return true;                                            // Retorna sin hacer nada
    
  if( _RedData.Red != NULL )                                // Si ya se habia creado una reducción anteriormente
    delete _RedData.Red;                                    // La libera
    
  CRedApi* Red = new CRedApi();                             // Crea una reducción nueva
  Red->SetDataPath( getDictPath() );                        // La pone el camino a los diccionarios
  
  if( !Red->Open(lng) )                                     // Trata de cargar todos sus diccionarios
    {
    delete Red;                                             // No pudo, libera el objeto reducción
    
    NSLog(@"No se pudo cargar la reduccón");
    return false;                                           // Retorna falso
    }
    
  _RedData.Red   = Red;                                     // Guarda la reducción
...
CStringA ReduceWord(const CStringA& aVerbo, int Lang)
  ...
  CRedDataApi* Dat = _RedData.Red->Reduce(aVerbo);          // Reduce la palabra
  if( !Dat ) return aVerbo;                                 // No la pudo reducir, retorna la misma palabra
  
  int count = Dat->Count();                                 // Obtiene la cantidad de reducciones
      
  for(int i=0; i<count; i++)                                // Recorre todas las reducciones
    {
    int nTipo = Dat->ClassType(i);                          // Obtiene tipo de la redicción actual
    ...
    if( nTipo==rVERBO )                                     // Si es verbo
      ...
      _RedData.Root = Dat->Root(i);                         // Guarda la raiz del verbo reducido
      break;                                                // Termina la busqueda
      }
    }
      
  delete Dat;                                               // Libera los datos de la reduccion
...
+(NSArray *) GetConjsByWord
  ...
  for( int i=0; i<_CData.Count; ++i )
    {
    NSAttributedString* Txt = [ProxyConj TextFromConj:i TwoLines:YES];
    ...
    if( HMaxConj < Txt.size.height ) HMaxConj = Txt.size.height;
    }
...
//  NSAttributedString* TxtHrd = [ProxyConj GroupHeader];
//  
...
//  [List addObject: obj];
  ...
  for( int i=3; i<_CData.Count; i += 6 )
    {
    NSAttributedString* Txt = [ProxyConj GroupModeTimeFrom:i];
...
+(NSArray *) GetConjsByPersons
  ...
  for( int i=0; i<6; ++i )
    {
    NSAttributedString* Txt = [ProxyConj GroupNumAndPerson:i];
...
+(NSAttributedString*) GetRootWord: (NSString *)sWord
  ...
  CRedDataApi* Dat = _RedData.Red->Reduce(aWord);                             // Reduce la palabra
  ...
  for(int i=0; i<count; i++)
    {
    CString Root      = Dat->Root     ( i );
    int     Mood      = Dat->Mood     ( i );
    int     Time      = Dat->Time     ( i );
    int     Person    = Dat->Person   ( i );
    int     Number    = Dat->Number   ( i );
    int     Gender    = Dat->Gender   ( i );
    ...
    if( nTipo==rVERBO )
      ...
      {
      [ProxyConj AddGenAndNumStr:Str Root:Root.c_str() Type:nTipo Gener:Gender Number:Number];
      }
    }
...
+(NSArray *) GetRootListForWord: (NSString *)sWord Lang:(int) lng
  {
