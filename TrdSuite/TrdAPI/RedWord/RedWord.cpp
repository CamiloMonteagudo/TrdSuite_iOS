...
//-------------------------------------------------------------------------------------------------------------------
// Clasifica el tipo de reducción
//-------------------------------------------------------------------------------------------------------------------
int CRWType::GetType()
  {
...
//-------------------------------------------------------------------------------------------------------------------
void CRedWord::SetGenDict( CDict* hGenDic, bool free )
	{
  if( m_GenDic && m_FreeGenDic )
    delete m_GenDic;
...
HREDDATA CRedWord::Reduce( const CString& Word  )
	...
	// Reducir compuestos  
	if( hData->m_Idioma == 3)   //Si es Alemán
		reducida = ReduceCompound(hData);
...
bool CRedWord::LoadGenDic()
  {
  CStringF DicPath;
  CStringF sLang(CLangIso::Name(m_Dicts->m_Lang));
  bool bFound = false;

  for( int i=0; i<CLangIso::Count(); ++i )
    {
    if (i != m_Dicts->m_Lang)
    	{
			DicPath = m_DataPath + sLang + LF"2" + CLangIso::Name(i) + LF".dic";
    	//TPtrC name(CLangIso::Name(i));
	    //DicPath.Format(L"%S%S2%S.dic", m_DataPath.GetDesC(), sLang.GetDesC(), &name );
  ...
  if (!bFound)
	...
	CDict *Dic = new CDict("Red_GenDict");
	//Dic->SetName(DicPath);

	//if( Dic->Load( DICC_FILE | DICC_READONLY ) )
		if( Dic->Open(DicPath) )
    {
    m_GenDic     = Dic;
    m_FreeGenDic = true;
  ...
  if(hData->m_Idioma != 3)
    ...
    for( numero=0; numero < 2; numero++ )
      ...
      do
        ...
        if(numero==1)
          {
          sustconc = CVConcordWordsIndex( m_hConj,root,MASCULINO,SINGULAR,MASCULINO,PLURAL,POSITIVO,&cwcount,cwidx);
          cwidx++;
          }
        else
          sustconc = root;
        ...
        for( decl=1; decl < 5; decl++ )
          ...
          WORD dnidx=0;
          WORD dncount=0;
          CStringA sustconcdec;
          do
            {
            sustconcdec = CVDeclineNounIndex( m_hConj,sustconc,MASCULINO,numero,decl,&dncount,dnidx);
            if( sustconcdec == hData->m_Palabra )
              {
              // Se agrega la reducción a la lista m_Types
              CRWType redtype;
				...
				for( numero=0; numero < 2; numero++ )
					for( decl=1; decl < 5; decl++ )
						for( artic=0; artic < 3; artic++ )
							...
							if(grado>0)
								adjconc = CVConcordWords(m_hConj,root,MASCULINO,SINGULAR,MASCULINO,SINGULAR,grado);
							else
								adjconc = root;
							...
							adjconc = CVDeclineAdjective( m_hConj,adjconc,genero,numero,decl,artic);
							//TRACE("2 %s == %s", adjconc.c_str(), hData->m_Palabra.c_str());
							if( adjconc == hData->m_Palabra )
								{
								// Se agrega la reducción a la lista m_Types
...
//tipo --> 0:Sust y Adj   1: Verbos
	...
	for(int j=0; j<2; j++ )
		...
		if(j==0 && hData->m_Idioma==3)
			{
			palorig = hData->m_Palabra;
			int pos = palorig.Find("ge");
			
			if(pos != -1)
				{
				palorig = palorig.Left(pos) + palorig.Mid(pos+2);
				inicial = palorig;
				}
			else
				return FALSE;
...
bool CRedWord::ReduceSimple( CRWhData* hData)
	...
	if( reducida && hData->m_Idioma == 3 )
		{
		int i = 0;
		bool inf = false;
		while( i < hData->m_Types.GetCount() )
			{
			if( (hData->m_Types[i].m_Modo == mINFINITIVO) &&  tipo_verbo.Find(hData->m_Types[i].m_Tipo) != -1)
				{
				inf = true;
				break;
				}
		...
		if(inf == true)
			{
			//Dejar una solo la reduccion en Infinitivo, Particip.
			int i=0;
			while( i < hData->m_Types.GetCount() )
				{
				int modo = hData->m_Types[i].m_Modo;
				if( modo == mINDICATIVO || modo == mIMPERATIVO || modo == mSUBJUNTIVO)
					{
					hData->m_Types.RemoveAt(i);
					}
				else
